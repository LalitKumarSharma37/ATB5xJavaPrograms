package src.basics_05_23;

public class Lab071 {
    public static void main(String[] args) {


        // Condition
        // -> If, If Else, If Else-If, Else, Switch ( BREAK)
        // -> CHeck for some condition, true or false
        // No Break and Continue- For If,If Else,If Else-If, Else
        // Switch ( BREAK) ->



        // Loops
        // -> Do perform a task multiple times
        // -> For, While, Do while,
        // Break and Continue

        // For, While and Do While

        // For Loop ->
        // 1. Use to perform a task multiple times
        // 2. Based on condition -> task multiple times.

        //Repetitive Task
        System.out.println("Pramod");
        //for(A,B,C)
        //A -> Initialization -> Means Tell JVM What variable you have, where you are starting point
        //B -> Condition -> When you want this to stop
        //C -> Increament / Decreament

        // A -> int i = 1
        //B -> i < = 10;
        //C -> i++ -> Post increament
        //D -> Which code -> System.out.println("Lalit");

        for(int i=0;i<=10;i++){
            System.out.println("Ram");
        }

    }
}
